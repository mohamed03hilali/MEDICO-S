const Type = require('./typeMedicament.model');
const { typeSchema, typeUpdateSchema } = require('./typeMedicament.validate');
...
async function getById(req, res) {
  try {
    ...
    if (!type) return res.status(404).json({ error: 'Type de médicament non trouvé' });
    res.json(type);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
